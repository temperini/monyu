(function(){
  ...
  /* ================= Modo conta nova (usuário zerado) =================
     ...
     demonstração populado — mantém v27 padrão idêntico ao v26 nesse fluxo. */
  ...
  function rangeLabelPromo(key,baseRange){
    ...
    if(!lo.discounted&&!hi.discounted)return fmt(baseRange)+' ficha'+(baseRange[1]>1?'s':'');
  ...
  function renderOpps(){
    var mh=oppMatched();
    $('#oppsHome').innerHTML=mh.length?mh.slice(0,3).map(oppCard).join('')
  ...
  $('#npSave').addEventListener('click',function(){
    ...
    PROJECTS[pid]={name:nome,short:nome.length>28?nome.substring(0,26)+'…':nome,complete:false,stage:'new'};
  ...
  var WS_SECS=[
    ...
    {t:'8. Orçamento e contrapartida',lim:2000,st:'draft',
     tx:'A Ada está elaborando esta seção. Ela usará os documentos financeiros do seu Conhecimento global para compor a contrapartida do projeto {{PROJETO}}. Previsão: hoje às 16h.'}
  ];
  ...
  if(isZeroMode)DOCS.length=0; /* conta nova: Conhecimento começa vazio, nada foi indexado ainda */
  ...
  /* ================= Animações ================= */
  syncSaldo();
  syncNavBadges(); /* garante que Meus projetos/Oportunidades/Conhecimento no menu nascem corretos — 0 no modo conta nova, reais no modo demo */
  requestAnimationFrame(function(){
    setTimeout(function(){
