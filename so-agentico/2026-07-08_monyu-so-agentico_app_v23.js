(function(){
  ...
  var TRAILS={
    ...
    adapratica:{name:'Ada na prática: da ideia ao texto pronto',ico:'✍️',gift:0,cat:'agentes',feat:true,lessons:[
    ...
    radar:{name:'Radar de captação além da subvenção',ico:'🧭',gift:0,cat:'recorrente',feat:true,lessons:[
    ...
    storytelling:{name:'Storytelling para bancas avaliadoras',ico:'🎤',gift:0,cat:'carreira',feat:true,lessons:[
  ...
  function trCardHTML(k,idx,opts){
    ...
    var like=trLikeGet(k),fav=trFavGet(k);
    var pct=trPct(k),label=n===0?'não iniciada':(n===L.length?'✓ trilha concluída':n+' de '+L.length+' aulas');
    var btnLabel=n===0?'Começar trilha':(n===L.length?'Rever trilha':'Continuar trilha');
    return '<article class="tr-card" data-trail="'+k+'" style="animation-delay:'+(0.04+(idx||0)*0.05)+'s">'+
      ...
      '</div>'+
      '<div class="tr-card-body">'+
        '<div class="tr-card-name"><b>'+T.name+'</b>'+(T.gift?'<span class="trail-gift">🎁 +'+T.gift+'</span>':'')+'</div>'+
  ...
  function finishOb(){if(obLvlTimer){clearInterval(obLvlTimer);obLvlTimer=null}obOv.classList.remove('open');document.body.style.overflow='';document.documentElement.style.overflow='';store.set('onboarded','1')}
