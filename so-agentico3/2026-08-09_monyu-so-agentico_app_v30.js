(function(){
  ...
  var accessMode=(function(){try{return new URLSearchParams(window.location.search).get('mode')||''}catch(e){return ''}})();
  var isZeroMode=accessMode==='zero';
  var isMvpMode=accessMode==='mvp';
  var isFreshAccountMode=isZeroMode||isMvpMode;
  var MVP_AGENT_KEYS=['rico','iris','ada'];
  function isAllowedAgent(key){return !isMvpMode||MVP_AGENT_KEYS.indexOf(key)>-1}
  if(isFreshAccountMode){
    if(isMvpMode)document.body.classList.add('mvp-account');
    document.body.classList.add('zero-account');
    try{
      var zmKeep=['monyu-so:theme','monyu-so:font','monyu-so:contrast','monyu-so:motion','monyu-so:sidebarCollapsed','monyu-so:install-visits','monyu-so:install-nudge-dismissed'];
      var zmRemove=[];
  ...
  function rwShowLevelUpModal(lv){
    if(typeof isMvpMode!=='undefined'&&isMvpMode)return;
  ...
  function rwLadderCheck(b){
    if(typeof isMvpMode!=='undefined'&&isMvpMode)return false;
  ...
  var MVP_MILESTONE_QUEUE=[],MVP_MILESTONE_OPEN=false,MVP_OPEN_ONBOARD_AFTER_MILESTONE=false;
  function mvpShowNextMilestone(){
  ...
  if(mvpMilestoneClose)mvpMilestoneClose.addEventListener('click',function(){
    close($('#mvpMilestoneModal'));MVP_MILESTONE_OPEN=false;
    if(MVP_OPEN_ONBOARD_AFTER_MILESTONE){
      MVP_OPEN_ONBOARD_AFTER_MILESTONE=false;
      setTimeout(function(){openOb(0)},180);
    }else setTimeout(mvpShowNextMilestone,180);
  });
    ...
    if(ul){
      ul.innerHTML=urgentes.map(function(j){
        ...
        return '<button class="notif-item urgent-item" data-urgent-jump="'+JORNADA.indexOf(j)+'">'+
          '<svg class="av" aria-hidden="true"><use href="'+insigniaHref(j.agent)+'"/></svg>'+
  ...
  var ACTIONS={
    ...
    write:{key:'ada',label:'Completar escrita com Ada',cost:20}
  };
  ...
  var PREMIUM_OPPS=[
    {name:'Capital Empreendedor',org:'Sebrae',kind:'Preparação e conexão com investidores',desc:'Apoio do planejamento à apresentação para investidores e à jornada de crescimento.',url:'https://sebrae.com.br/subsites/capital-empreendedor',logo:'https://sebrae.com.br/content/dam/portal-sebrae/na/pt/imagens/logo/logo-sebrae.svg',logoBg:'#fff'},
    {name:'Iaris Ventures',org:'Iaris Ventures',kind:'Venture Builder',desc:'Seleção e desenvolvimento de startups com acesso a mercado, governança, networking e fundraise.',url:'https://iaris.ventures/cadastre-sua-startup/',logo:'https://iaris.ventures/wp-content/uploads/2024/05/logo-iaris.webp',logoBg:'#fff'},
    {name:'InvestPlus',org:'InvestPlus',kind:'Investimento privado',desc:'Plataforma de equity crowdfunding com ofertas de startups e negócios inovadores.',url:'https://investplus.vc/',logo:'https://investplus.vc/assets/logo-white-CpCnWojn.png',logoBg:'#171717'},
    {name:'da.tes',org:'CESAR',kind:'Conexões e oportunidades',desc:'Conecta startups a investidores, empresas, chamadas de inovação aberta e editais.',url:'https://dates.cesar.org.br/',logo:'https://dates.cesar.org.br/_next/static/media/logo-dates-white.04ac2156.svg',logoBg:'#161637'}
  ];
  var IRIS_COST=5;
  var MVP_OPP_UNLOCKED=0;
  ...
  function renderNextBest(){
    ...
    box.className='nba';box.hidden=false;box.style.display='grid';
    box.style.setProperty('--ac','var(--c-'+rec.agent+')');
    box.innerHTML=
      '<svg class="nba-av" aria-hidden="true"><use href="'+insigniaHref(rec.agent)+'"/></svg>'+
      '<div class="nba-body">'+
  ...
  function showView(name){
    if(isMvpMode&&['inicio','aprovacoes','projetos','radar','agentes','perfil','empresa','config'].indexOf(name)===-1)name='inicio';
    ...
    /* Entrar no Radar não muda o estado de novidade. Uma oportunidade só é
       considerada vista depois que o usuário abre os detalhes dela. */
  ...
  function globalAgentNames(items){
    ...
    items.forEach(function(item){
      var key=item.dataAgent||item.agent||item;
      var name=AG[key]&&AG[key].name;
  ...
  function openGlobalActivity(){
    trayMin=false;
    updateTray();
    if(tray&&tray.classList.contains('show'))tray.scrollIntoView({behavior:'smooth',block:'nearest'});
  }
  function renderGlobalStatusGuide(){
    ...
    guide.hidden=false;
    guide.className='global-status-guide is-'+state;
    guide.innerHTML='<button type="button" class="global-status-action" data-global-status-action="'+action+'"><span class="global-status-dot" aria-hidden="true"></span><span class="global-status-copy">'+copy+'</span><span class="global-status-link">'+link+'</span></button>';
  }
  ...
  function syncSidebarBrandControls(){
    ...
    if(logoBtn){
      logoBtn.setAttribute('aria-label',collapsed?'Expandir menu lateral':'Ir para Início');
      logoBtn.setAttribute('data-tip',collapsed?'Expandir menu':'Início');
    }
  ...
  if(logoBtn)logoBtn.addEventListener('click',function(e){
    ...
    if(isDesktopSidebar()){
      ...
      if(collapsed){setSidebarCollapsed(false);return}
    }
    showView('inicio');
  });

  /* No rail recolhido os nomes continuam disponíveis para tecnologias
     assistivas e para o tooltip nativo. Rótulo visual fora do menu não é
     navegação: gera sobreposição, não funciona em toque e compete com a tela. */
  $$('.nav-item[data-tip]').forEach(function(item){
    var label=item.getAttribute('data-tip');
  ...
  /* Mensagens flutuantes de rodapé foram desativadas em todas as versões.
     As chamadas permanecem neutras para preservar os fluxos existentes enquanto
     cada resposta é apresentada no contexto correspondente da interface. */
  function toast(){}
  document.addEventListener('click',function(e){
  ...
  if(isFreshAccountMode)banner.classList.add('hidden'); /* banner estático não se aplica a quem ainda não tem projeto */
  ...
  function openRun(key,ctxProj){
    if(!isAllowedAgent(key)){
      toast('Este Acesso Beta Antecipado inclui somente Rico, Íris e Ada.');
      return;
    }
    ...
    $('#runInsignia').querySelector('use').setAttribute('href',insigniaHref(key));
  ...
  function confirmRun(){
    ...
    if(pendKey==='rico'&&isFreshAccountMode&&!isMvpMode)ensureSyntheticMatches(ids);
    if(isMvpMode)ids.forEach(function(pid){mvpDismissRecommendation(pid,pendKey)});
  ...
  function oppMarkSeen(id){
    var seen=oppSeenIds();
    if(seen.indexOf(id)===-1){seen.push(id);store.set('oppsSeen',JSON.stringify(seen))}
  }
  function orderOppsBySeen(list,seen){
    return list.slice().sort(function(a,b){
      return (seen.indexOf(a.id)!==-1)-(seen.indexOf(b.id)!==-1);
    });
  }
  ...
  function mvpOpportunityCta(){
    ...
    return '<article class="opp opp-premium-empty"><span class="premium-empty-icon">＋</span><span class="opp-cat">Encontre mais oportunidades</span><h3>'+(hasProjects?'Busque oportunidades para suas ideias/projetos':'Comece pela sua ideia/projeto')+'</h3><span class="premium-desc">'+(hasProjects?'O Rico pesquisa novas oportunidades aderentes depois que você confirma o consumo de fichas.':'Cadastre uma ideia/projeto para o Rico poder buscar oportunidades personalizadas para você.')+'</span><button class="btn-primary" '+(hasProjects?'data-agent="rico"':'data-inicio-newproj')+'>'+(hasProjects?'Executar o Rico · a partir de 1 ficha':'Cadastrar projeto')+'</button></article>';
  }
  function oppMatched(){return visibleOpps().filter(function(o){return o.matches.length>0})}
  function oppUnmatched(){return visibleOpps().filter(function(o){return o.matches.length===0})}
  var oppFilter='todas';
  function renderOpps(){
    /* NOVO permanece até a abertura do detalhe. Dentro de cada grupo, a ordem
       original é preservada: novos primeiro e oportunidades já abertas depois. */
    var seenBefore=oppSeenIds();
    function isNewOpp(o){return seenBefore.indexOf(o.id)===-1}
    var mh=orderOppsBySeen(oppMatched(),seenBefore);
  ...
  function renderInicioZero(){
    ...
    if(isFreshAccountMode){
      if(isMvpMode&&wrap){
        ...
        wrap.style.display=hasProj?'':'none';
      }else if(wrap)wrap.style.display='none';
      if(empty){
        empty.style.display=hasProj?'none':'';
        empty.innerHTML='<p style="font-size:.86rem;color:var(--text-3);margin-bottom:1rem">Você ainda não tem ideias/projetos cadastrados. Cadastre sua primeira ideia/projeto e a MonyU te guia até a submissão.</p><button class="btn-primary" data-inicio-newproj style="font-size:.8rem">+ Cadastrar meu primeiro projeto</button>';
      }
    }else{
  ...
  function aprovSorted(){
    return getPendingApprovals().map(function(j){return {j:j,idx:JORNADA.indexOf(j)}}).sort(function(a,b){
  ...
  function aprovRow(entry){
    ...
    if(j.dataAgent){
      ...
      return '<div class="aprov-row" data-aprov="'+idx+'" data-aprov-agent="'+j.dataAgent+'" data-cost-min="'+fMin+'" data-cost-max="'+fMax+'">'+
  ...
  var AUTO_CAPABLE={rico:true,iris:true,ada:true};
  var AUTO_MODE={active:false,limit:0,used:0,agents:['rico','iris','ada']};
  var APROV_EXEC_ORIGIN='manual';
  function autoAgentAllowed(agentKey){
    return AUTO_MODE.active&&AUTO_CAPABLE[agentKey]===true&&AUTO_MODE.agents.indexOf(agentKey)>-1;
  }
  function autoModeApply(){
    if(!AUTO_MODE.active)return;
    var rows=$$('#aprovList .aprov-row[data-aprov]');
    var total=0,selected=0,skipped=0,available=Math.max(0,Math.min(AUTO_MODE.limit-AUTO_MODE.used,saldo));
    rows.forEach(function(row){
      var cb=row.querySelector('[data-aprovcb]');
      if(!cb)return;
      if(row.getAttribute('data-auto')!=='1'){cb.checked=false;return}
      var cost=parseInt(row.getAttribute('data-cost-max'),10);
      if(total+cost<=available){cb.checked=true;total+=cost;selected++}
      else{cb.checked=false;skipped++}
    });
    refreshAprov();
    var lim=$('#autoLimitTxt');if(lim)lim.textContent=AUTO_MODE.limit;
    var used=$('#autoUsed');if(used)used.textContent=AUTO_MODE.used+total;
    if(skipped>0){
  ...
  if(autoConfirmBtn2)autoConfirmBtn2.addEventListener('click',function(){
    ...
    var permitted=Array.prototype.slice.call($$('#autoModal [data-auto-agent]:checked')).map(function(cb){return cb.getAttribute('data-auto-agent')});
    if(!permitted.length){toast('Selecione ao menos um agente permitido para o modo automático.');return}
    AUTO_MODE.active=true;AUTO_MODE.limit=v;AUTO_MODE.used=0;AUTO_MODE.agents=permitted;
    close($('#autoModal'));
    syncAutoModeUi();
    addNotif('rico','<b>Modo automático ativado.</b> As recomendações serão executadas dentro do limite de '+v+' fichas.','aprovacoes');
    renderAprov();
    runAutomaticApprovals();
  ...
  function syncAutoModeUi(){
    ...
    /* "Selecionar todas" é controle de lote, não CTA persistente. Sem
       execução recomendada, mantê-lo visível sugere uma ação que não existe. */
    var hasRunnableApproval=getPendingApprovals().length>0;
    if(isMvpMode){
      AUTO_MODE.active=false;
      if(bar)bar.style.display='none';
      if(activate){activate.style.display='';setMvpAutomaticComingSoon()}
      if(selectAll)selectAll.style.display=hasRunnableApproval?'':'none';
    ...
    if(selectAll)selectAll.style.display=(!AUTO_MODE.active&&hasRunnableApproval)?'':'none';
    if(title)title.textContent=AUTO_MODE.active?'Execuções automáticas e manuais':'Ações recomendadas';
    var agentsTxt=$('#autoAgentsTxt');
    if(agentsTxt)agentsTxt.textContent=AUTO_MODE.agents.map(function(k){return AG[k]&&AG[k].name||k}).join(', ');
  }
  function renderAprov(){
    ...
    if(list){
      ...
      if(AUTO_MODE.active){
        var automatic=items.filter(function(entry){return entry.j.dataAgent&&autoAgentAllowed(entry.j.dataAgent)});
        var manual=items.filter(function(entry){return !entry.j.dataAgent||!autoAgentAllowed(entry.j.dataAgent)});
        list.innerHTML=(automatic.length?'<div class="aprov-group-head"><span class="origin-badge origin-auto">AUTO</span><small>Execução dentro das permissões e do limite definido</small></div>'+automatic.map(aprovRow).join(''):'')+
          (manual.length?'<div class="aprov-group-head manual"><span class="origin-badge origin-manual">MANUAL</span><small>Agentes fora das permissões do modo automático</small></div>'+manual.map(aprovRow).join(''):'');
        automatic.forEach(function(entry){var row=list.querySelector('[data-aprov="'+entry.idx+'"]');if(row)row.setAttribute('data-auto','1')});
        manual.forEach(function(entry){var row=list.querySelector('[data-aprov="'+entry.idx+'"]');if(row)row.setAttribute('data-auto','0')});
      }else{
  ...
  if(aprovConfirmBtn)aprovConfirmBtn.addEventListener('click',function(){
    ...
    var rows=$$('#aprovList .aprov-row[data-aprov]');
    var total=0,started=0,doneIdx=[],anyPromo=false,ranRico=false,execOrigin=APROV_EXEC_ORIGIN;
    rows.forEach(function(row){
      var cb=row.querySelector('[data-aprovcb]');
      ...
      startTask(j.dataAgent,AG[j.dataAgent].name+' - '+AGLABEL[j.dataAgent]+' · '+pname.substring(0,36)+(pname.length>36?'…':''),AG[j.dataAgent].name+' concluiu: '+AGLABEL[j.dataAgent]+' ('+pname.substring(0,26)+(pname.length>26?'…':'')+')',8000+Math.random()*6000,[j.proj],execOrigin);
    ...
    if(started===0)return;
    if(execOrigin==='auto')AUTO_MODE.used+=total;
    APROV_EXEC_ORIGIN='manual';
    if(typeof rwRecalcAll==='function')rwRecalcAll();
  ...
  function runAutomaticApprovals(){
    if(!AUTO_MODE.active)return;
    autoModeApply();
    $$('#aprovList .aprov-row[data-auto="0"] [data-aprovcb]').forEach(function(cb){cb.checked=false});
    if(!$$('#aprovList .aprov-row[data-auto="1"] [data-aprovcb]:checked').length)return;
    APROV_EXEC_ORIGIN='auto';
    $('#aprovConfirm').click();
  }
  renderAprov();
  ...
  function mvpUnlockRicoOpportunities(pid){
    if(!isMvpMode)return;
    MVP_OPP_UNLOCKED=Math.min(OPPS.length,MVP_OPP_UNLOCKED+3);
    visibleOpps().forEach(function(o){
      if(!o.matches.some(function(m){return m.p===pid}))o.matches.push({p:pid,pct:58+Math.floor(Math.random()*35)});
    });
    renderOpps();syncNavBadges();
  }
  ...
  function openOpp(id){
    ...
    '<div class="osec" id="oppActionSec" style="margin-bottom:0"><h4>Seus projetos com match - escolha uma ou mais ações</h4>'+
      '<p class="osec-hint"><b>Você no controle.</b> Selecione um ou mais agentes para cada ideia/projeto - nada roda sem a sua confirmação.</p>'+
      '<div class="match-select">'+
      o.matches.map(function(m,i){
        var proj=PROJECTS[m.p];
        var rec=rowRecommendedAct(proj);
        var altKey=proj.complete?'adapt':(isMvpMode?'write':'psf');
        if(isMvpMode&&rec==='psf')rec='write';
        var actKeys=['diag',altKey].sort(function(a,b){return (rec===a?-1:rec===b?1:0)});
        return '<div class="match-project" data-mrow="'+m.p+'">'+
          '<div class="match-project-head"><span><b>'+proj.name+'</b><small>'+(proj.complete?'Ideia/projeto 100% elaborada na plataforma':'Ideia/projeto em construção')+' · match com os critérios do edital</small></span><span class="match-score"><small>Aderência</small><b class="mr-pct '+pctClass(m.pct)+'">'+m.pct+'%</b></span></div>'+
          '<div class="match-agent-list">'+actKeys.map(function(actKey){
            var act=ACTIONS[actKey],isRec=rec===actKey;
            return '<label class="match-agent-row'+(isRec?' rec':'')+'">'+
              '<input type="checkbox" data-act-choice="'+actKey+'" aria-label="Selecionar '+act.label+'">'+
              '<svg class="av" aria-hidden="true"><use href="#ins-'+act.key+'"/></svg>'+
              '<span class="match-agent-copy"><b>'+AG[act.key].name+'</b><small>'+act.label+'</small></span>'+
              '<span class="match-agent-cost">'+costBadgeHTML(act.key,act.cost)+'</span>'+
              (isRec?'<span class="act-rec-tag">Recomendado</span>':'')+
            '</label>';
          }).join('')+'</div>'+
        '</div>';
      }).join('')+
    ...
    open(oppModal);
    var rows=oppBox.querySelectorAll('.match-project');
    function rowActs(row){return Array.from(row.querySelectorAll('[data-act-choice]:checked')).map(function(c){return c.getAttribute('data-act-choice')})}
    function refresh(){
      var n=0,total=0;
      rows.forEach(function(row){
        var actKeys=rowActs(row);
        row.classList.toggle('checked',actKeys.length>0);
        actKeys.forEach(function(actKey){
          var act=ACTIONS[actKey];
          n++;total+=applyPromo(act.key,act.cost).final;
        });
      });
      ...
      if(n===0){btn.disabled=true;btn.removeAttribute('data-buy');btn.textContent='Selecione ao menos 1 ação'}
      else if(total>saldo){
        var g=total-saldo;
    ...
    rows.forEach(function(row){
      row.querySelectorAll('[data-act-choice]').forEach(function(cb){
    ...
    oppBox.querySelectorAll('[data-mp-jump]').forEach(function(btn){
      btn.addEventListener('click',function(){
        ...
        var row=oppBox.querySelector('.match-project[data-mrow="'+pid+'"]');
        if(!row)return;
    ...
    oppBox.querySelector('#oppRun').addEventListener('click',function(){
      var buyGap=this.getAttribute('data-buy');
      if(buyGap){resolverSaldo(parseInt(buyGap,10),'Você também pode desmarcar uma ou mais ações para caber no saldo atual.',refresh);return}
      var total=0,started=0;
      rows.forEach(function(row){
        var pid=row.getAttribute('data-mrow');
        var pname=PROJECTS[pid].name;
        rowActs(row).forEach(function(actKey){
          var act=ACTIONS[actKey];
          total+=applyPromo(act.key,act.cost).final;started++;
          if(isMvpMode)mvpDismissRecommendation(pid,act.key);
          startTask(act.key,act.label+' - '+pname.substring(0,40)+'… ('+o.abbr+')',act.label+' concluída - '+o.abbr,8000+Math.random()*6000,[pid]);
  ...
  function syncNotifBadge(){
    var unread=$$('#notifList .notif-item:not(.read)').length;
    var dot=$('#notifDot'),btn=$('#notifBtn');
    if(dot)dot.style.display=unread?'':'none';
    if(btn)btn.setAttribute('aria-label','Abrir notificações ('+unread+' não lida'+(unread===1?'':'s')+')');
  }
  function addNotif(agentKey,html,nav){
    var b=document.createElement('button');
    b.className='notif-item';
    if(nav)b.setAttribute('data-nav',nav);
  ...
  function mvpHomeEvent(evento){
    ...
    MVP_HOME_EVENTOS=MVP_HOME_EVENTOS.slice(0,6);
  }
  ...
  function mvpDismissRecommendation(pid,agent){
    if(!isMvpMode)return;
    for(var i=JORNADA.length-1;i>=0;i--){if(JORNADA[i].mvpGenerated&&JORNADA[i].proj===pid&&JORNADA[i].dataAgent===agent)JORNADA.splice(i,1)}
  ...
  var EXECUTION_LOG=[];
  var EXECUTION_LOG_SEQ=0;
  function executionEscape(value){
    return String(value==null?'':value).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');
  }
  function renderExecutionLog(){
    var host=$('#executionLog');if(!host)return;
    if(!EXECUTION_LOG.length){host.innerHTML='<p class="execution-log-empty">Nenhum agente concluiu uma execução nesta sessão.</p>';return}
    host.innerHTML=EXECUTION_LOG.map(function(entry){
      return '<article class="exec-log-row">'+
        '<div class="exec-log-agent"><svg class="av" aria-hidden="true"><use href="'+insigniaHref(entry.agent)+'"/></svg><span><b>'+executionEscape(entry.agentName)+'</b><small class="origin-badge '+(entry.origin==='AUTO'?'origin-auto':'origin-manual')+'">'+executionEscape(entry.origin)+'</small></span></div>'+
        '<div class="exec-log-meta"><b>'+executionEscape(entry.user)+'</b> · '+executionEscape(entry.date)+' às '+executionEscape(entry.time)+' · '+executionEscape(entry.project)+'</div>'+
        '<a class="exec-log-result" href="#resultado-'+entry.id+'" data-execution-result="'+entry.id+'">Ver resultado</a>'+
      '</article>';
    }).join('');
  }
  function recordExecution(agentKey,projectIds,origin,doneLabel){
    var now=new Date();
    var projects=(projectIds||[]).map(function(pid){return PROJECTS[pid]&&PROJECTS[pid].short||'Ideia/projeto'}).join(', ')||'Sem ideia/projeto vinculada';
    EXECUTION_LOG.unshift({
      id:String(++EXECUTION_LOG_SEQ),agent:agentKey,agentName:AG[agentKey]&&AG[agentKey].name||agentKey,
      user:'Evandro Temperini',origin:origin==='auto'?'AUTO':'MANUAL',date:now.toLocaleDateString('pt-BR'),
      time:now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),project:projects,result:doneLabel
    });
    EXECUTION_LOG=EXECUTION_LOG.slice(0,10);
    renderExecutionLog();
  }
  document.addEventListener('click',function(e){
    var link=e.target.closest('[data-execution-result]');if(!link)return;
    e.preventDefault();
    var entry=EXECUTION_LOG.filter(function(item){return item.id===link.getAttribute('data-execution-result')})[0];
    if(!entry)return;
    if(entry.agent==='rico')showView('radar');
    else if(entry.agent==='ada')openWs();
    else toast(entry.result+' - visualização completa em breve no piloto. (demo)');
  });
  renderExecutionLog();
  function startTask(agentKey,label,doneLabel,dur,projectIds,executionOrigin){
    executionOrigin=executionOrigin||'manual';
    if(agentKey==='rico'||agentKey==='iris'||agentKey==='ada'){
      mvpCelebrate('first-'+agentKey,'Primeira execução de '+((AG[agentKey]&&AG[agentKey].name)||agentKey)+'!','Você deu mais um passo na jornada de captação da sua ideia/projeto.','✨',agentKey);
    }
    dur=dur||9000;activeCount++;
    (projectIds||[]).forEach(function(pid){MVP_RUNNING[mvpRunKey(pid,agentKey)]=true});
    var item=document.createElement('div');
    item.className='tray-item';
    item.innerHTML='<svg class="av" aria-hidden="true"><use href="#ins-'+agentKey+'"/></svg>'+
      '<div class="t-body"><div class="t-label"><span class="t-label-text"></span>'+(executionOrigin==='auto'?'<span class="tray-auto-badge">AUTO</span>':'')+'</div><div class="tray-prog"><i></i></div></div>';
    item.querySelector('.t-label-text').textContent=label;
    ...
    setTimeout(function(){
      ...
      item.querySelector('.t-label-text').textContent=doneLabel;
      var view=document.createElement('button');view.className='t-view';view.textContent='Ver resultado';
      view.addEventListener('click',function(){
        if(agentKey==='rico')showView('radar');
        else if(agentKey==='ada')openWs();
      ...
      (projectIds||[]).forEach(function(pid){delete MVP_RUNNING[mvpRunKey(pid,agentKey)]});
      addNotif(agentKey,'<b>'+doneLabel+'</b>. O resultado já está disponível para você.');
      recordExecution(agentKey,projectIds,executionOrigin,doneLabel);
      if(isMvpMode&&Array.isArray(MVP_HOME_EVENTOS)){
        var autoExecution=executionOrigin==='auto';
        mvpHomeEvent({ag:agentKey,txt:'<b>'+doneLabel+'</b>',why:(autoExecution?'Execução autorizada pelo modo automático':'Execução solicitada por você no protótipo')+'. As novas informações foram incorporadas à ideia/projeto.',org:autoExecution?'auto':'voce',orgTxt:autoExecution?'AUTO':'MANUAL',custo:'conforme confirmação',quando:'agora'});
        var agIx=AGD.map(function(a){return a.k}).indexOf(agentKey);
        if(agIx>-1){
          AGD[agIx].hist.unshift({d:'agora',t:doneLabel});
          var agHist=document.querySelector('.agent-big[data-ag="'+agIx+'"] .ag-hist');
          if(agHist){agHist.classList.remove('muted');agHist.innerHTML='<small>Última atividade</small>'+doneLabel+' <span class="hd">· agora</span>'}
        }
        (projectIds||[]).forEach(function(pid){mvpRefreshJourney(pid,'agent_completed',agentKey)});
        if(typeof renderHomeAtividade==='function')renderHomeAtividade();
        if(typeof renderAgentsState==='function')renderAgentsState();
      }
  ...
  $('#buyConfirm').addEventListener('click',function(){
    ...
    mvpCelebrate('first-purchase','Primeira compra de fichas!','Seu saldo foi reforçado e você já pode avançar com novas execuções.','🪙');
  });
  ...
  function npSetMode(m){
    ...
    var policy=$('#npDocsPolicy');if(policy&&isMvpMode)policy.textContent='Os arquivos ficam vinculados a esta ideia/projeto e aparecem somente como histórico na Memória.';
  }
  document.addEventListener('click',function(e){
  ...
  function npAddDoc(name,ext,type){
    ...
    row.className='np-doc'+(isMvpMode?' np-doc-simple':'');
    row.innerHTML='<span class="kd-ico">'+ext+'</span><span class="nd-name"></span>'+ 
      (isMvpMode?'':'<button class="np-type" data-cycle data-tip="Clique para trocar o tipo">'+type+'</button><label class="np-scope" data-tip="Padrão: somente este projeto"><span class="switch kb-scope-switch"><input type="checkbox" data-scope><span class="track-sw"></span><span class="thumb"></span></span><span>Global</span></label>')+ 
    ...
    var cycle=row.querySelector('[data-cycle]');if(cycle)cycle.addEventListener('click',function(){
      var ix=NP_TYPES.indexOf(this.textContent);
      this.textContent=NP_TYPES[(ix+1)%NP_TYPES.length];
    });
    var scope=row.querySelector('[data-scope]');if(scope)scope.addEventListener('change',function(){
  ...
  $('#npSave').addEventListener('click',function(){
    ...
    if(!nome){toast('Dê um nome à ideia/projeto antes de salvar');$('#npNome').focus();return}
    projSeq++;
    ...
    docRows.forEach(function(row){
      DOCS.push({
        ...
        type:row.querySelector('[data-cycle]')?row.querySelector('[data-cycle]').textContent:'Arquivo',
        orig:'upload',date:hoje,use:'-',
        proj:row.querySelector('[data-scope]')&&row.querySelector('[data-scope]').checked?null:pid
      });
    });
    ...
    card.innerHTML=
      '<div class="proj-head">'+
        ...
        '<div class="proj-title"><b></b><span data-project-caption="'+pid+'">'+(imported?'Projeto importado':'Projeto cadastrado')+' · Tema: '+tema+' · criado em '+hoje+(nDocs?' · '+nDocs+' documento(s) na Memória':'')+'</span></div>'+ 
        '<span data-project-state="'+pid+'">'+projectStateHTML(pid)+'</span>'+ 
      '</div>'+ 
      projectJourneyHTML(pid)+
      '<div class="project-usage-row">'+
        '<span class="project-usage-pill" data-project-spent="'+pid+'" title="Total de fichas consumidas pelos agentes nesta ideia/projeto"><span class="ficha-coin" aria-hidden="true">M</span><b>0</b> fichas usadas por agentes</span>'+ 
      '</div>'+ 
    ...
    if(isFreshAccountMode&&typeof wsApplyProjName==='function')wsApplyProjName(nome);
    if(isMvpMode)mvpRefreshJourney(pid,'created');
  ...
  /* ================= Meus Agentes · identidade visual v30 ================= */
  function insigniaHref(k){return '#ins-'+k}
  function avatarSrc(k){return 'assets/personas/avatars/'+k+'.png'}
  function decorateSingleAgentCta(btn,key){
    if(!btn||!key||!AG[key])return;
    var old=btn.querySelector('.agent-cta-insignia');
    if(old)old.remove();
    var svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
    svg.setAttribute('class','agent-cta-insignia');svg.setAttribute('aria-hidden','true');
    var use=document.createElementNS('http://www.w3.org/2000/svg','use');
    use.setAttribute('href',insigniaHref(key));svg.appendChild(use);
    btn.insertBefore(svg,btn.firstChild);btn.classList.add('agent-cta');
    btn.style.setProperty('--agent-cta-color','var(--c-'+key+')');
  }
  function decorateAgentCtas(scope){
    scope=scope||document;
    var nodes=[];
    if(scope.matches&&scope.matches('button[data-agent],a[data-agent],button[data-ns-agent],button[data-oppmatch]'))nodes.push(scope);
    if(scope.querySelectorAll)nodes=nodes.concat(Array.from(scope.querySelectorAll('button[data-agent],a[data-agent],button[data-ns-agent],button[data-oppmatch]')));
    nodes.forEach(function(btn){
      var key=btn.getAttribute('data-agent')||btn.getAttribute('data-ns-agent')||(btn.hasAttribute('data-oppmatch')?'rico':'');
      if(!btn.querySelector('.agent-cta-insignia'))decorateSingleAgentCta(btn,key);
    });
  }
  function decorateFreeProjectCtas(scope){
    scope=scope||document;
    var selector='[data-inicio-newproj],#runProjNew,#npSave,#obZeroBtn,#btnNewProj,#inicioEmptyNewProj,#projListEmptyNewProj';
    var nodes=[];
    if(scope.matches&&scope.matches(selector))nodes.push(scope);
    if(scope.querySelectorAll)nodes=nodes.concat(Array.from(scope.querySelectorAll(selector)));
    nodes.forEach(function(btn){
      if(btn.querySelector('.free-action-badge'))return;
      var badge=document.createElement('span');badge.className='free-action-badge';badge.textContent='Grátis';
  ...
  if(isFreshAccountMode)AGD.forEach(function(a){a.hist=[]}); /* conta nova: nenhum agente rodou ainda */
  if(isMvpMode){
    document.title='MonyU - Acesso Beta Antecipado';
    AGD=AGD.filter(function(a){return isAllowedAgent(a.k)});
    var ricoMvp=AGD.filter(function(a){return a.k==='rico'})[0],adaMvp=AGD.filter(function(a){return a.k==='ada'})[0];
    if(ricoMvp)ricoMvp.faq[2][1]='De fontes oficiais de fomento monitoradas pela MonyU.';
    if(adaMvp)adaMvp.does[2]='Aplica padrões de qualidade validados pela MonyU e sinaliza quando faltam informações do projeto.';
    ['rotinas','atividade','conhecimento','resultados','rewards','giga','trein'].forEach(function(view){
      $$('[data-nav="'+view+'"]').forEach(function(el){el.hidden=true;el.style.display='none'});
    });
    var mvpCta=$('#ctaSpec');if(mvpCta){mvpCta.hidden=false;mvpCta.style.display='';mvpCta.setAttribute('data-tip','Conversar sobre a assessoria especializada da MonyU');var mvpCtaLabel=mvpCta.querySelector('.cta-spec-label');if(mvpCtaLabel)mvpCtaLabel.textContent='Captar com especialista'}
    var mvpTag=$('#navAgentsTag');if(mvpTag)mvpTag.textContent='3';
    var mvpRw=$('#rwChip');if(mvpRw)mvpRw.style.display='none';
    var mvpResumo=$('#btnResumo');if(mvpResumo)mvpResumo.style.display='none';
    var mvpRoi=document.querySelector('#view-inicio .home-roi');if(mvpRoi)mvpRoi.style.display='none';
    var mvpModo=$('#modoHint');if(mvpModo)mvpModo.style.display='none';
    var mvpHomeAgents=document.querySelector('#view-inicio .home-agentes');if(mvpHomeAgents)mvpHomeAgents.style.display='block';
    var mvpHomeOpps=document.querySelector('#view-inicio .home-opps');if(mvpHomeOpps)mvpHomeOpps.style.display='block';
    var mvpHomeFinance=document.querySelector('#view-inicio .home-financeiro');if(mvpHomeFinance)mvpHomeFinance.style.display='grid';
    var mvpSupport=document.querySelector('#view-inicio section[aria-label="Agentes de apoio"]');if(mvpSupport)mvpSupport.style.display='none';
    $$('#view-inicio .home-agentes article.agent').forEach(function(card){
      var run=card.querySelector('[data-agent]'),key=run&&run.getAttribute('data-agent');
      if(!key||!isAllowedAgent(key))card.style.display='none';
      else if(key==='ada'){var step=card.querySelector('.step-num');if(step)step.textContent='3'}
    });
    ...
    var mvpFichaNums=document.querySelector('#view-inicio .fichas-nums');if(mvpFichaNums)mvpFichaNums.innerHTML='<b><span class="saldo-view">30</span> de 30 fichas</b><span>renovam mensalmente</span>';
    var mvpFichaBar=document.querySelector('#view-inicio .fichas-bar');if(mvpFichaBar){mvpFichaBar.setAttribute('aria-valuemax','30');mvpFichaBar.setAttribute('aria-label','30 de 30 fichas disponíveis')}
    var mvpFichaDetail=document.querySelector('#view-inicio .fichas-detail');if(mvpFichaDetail)mvpFichaDetail.innerHTML='<div><span>Benefício mensal de lançamento</span><b>30 fichas grátis</b></div><div><span>Compra de fichas adicionais</span><b>50% de desconto</b></div><div><span>Valor promocional por ficha</span><b>R$ 1,75</b></div>';
    var mvpBuySub=document.querySelector('#buyModal .sub');if(mvpBuySub)mvpBuySub.innerHTML='No <b>Acesso Antecipado</b>, você recebe 30 fichas gratuitas por mês e compra fichas adicionais com <b>50% de desconto</b> durante o lançamento.';
    var mvpPackData=[[10,17.5],[25,43.75],[50,87.5],[100,175]];
    ...
    if(mvpWsSpec){
      mvpWsSpec.textContent='Conversar sobre assessoria especializada';
      var mvpWsTip=mvpWsSpec.closest('.ws-tip');
  ...
  agGrid.innerHTML=AGD.map(function(a,i){
    ...
    return '<article class="agent-big'+(a.soon?' soon':'')+(a.gate?' gate':'')+'" data-ag="'+i+'" style="--ac:var(--c-'+a.k+');animation-delay:'+(0.03+i*0.04)+'s" tabindex="0" role="button" aria-label="Ver detalhes de '+a.name+'">'+
      '<div class="agent-head"><svg class="av" aria-hidden="true"><use href="'+insigniaHref(a.k)+'"/></svg>'+
  ...
  function projectJourneyHTML(pid){
    var ran=PROJ_RAN[pid]||[],project=PROJECTS[pid]||{};
    /* A jornada pertence ao projeto. Agentes são participantes possíveis de
       cada etapa e o array aceita quantos forem necessários, inclusive em
    ...
    var steps=isMvpMode
      ...
      : [
          {k:'discovery',n:'Estruturar oportunidade',agents:['aurora','barto','mike'],done:false,optional:true},
          {k:'opportunities',n:'Mapear oportunidades',agents:['rico'],done:ran.indexOf('rico')>-1},
          {k:'potential',n:'Avaliar potencial',agents:['iris'],done:ran.indexOf('iris')>-1},
          {k:'proposal',n:'Elaborar proposta',agents:['ada'],done:project.stage==='elaborated'||project.stage==='submitted'||project.stage==='complete'},
          {k:'validation',n:'Validar proposta',agents:['banca'],done:ran.indexOf('banca')>-1,optional:true},
          {k:'submission',n:'Submeter proposta',people:['Revisão humana'],done:project.stage==='submitted'||project.stage==='complete'}
        ];
    steps=steps.filter(function(s){return !s.optional||s.done||s.recommended});
    var required=steps.filter(function(s){return !s.optional}),nextKey=(required.filter(function(s){return !s.done})[0]||{}).k;
    function participants(step){
      var names=(step.agents||[]).filter(isAllowedAgent).map(function(k){return '<span class="step-agent"><svg aria-hidden="true"><use href="'+insigniaHref(k)+'"/></svg>'+nomeAgente(k)+'</span>'});
      (step.people||[]).forEach(function(name){names.push('<span class="step-person">'+name+'</span>')});
      return names.join('');
    }
    return '<div class="stepper project-stepper project-stage-timeline" data-project-timeline="'+pid+'" role="list" aria-label="Etapas para chegar à submissão">'+steps.map(function(s,i){
      var cl=s.done?'done':(s.k===nextKey?'current':'');
      return '<div class="step '+cl+'" role="listitem"'+(s.k===nextKey?' aria-current="step"':'')+'><span class="step-dot">'+(s.done?'✓':(i+1))+'</span><span class="step-name">'+s.n+'</span><span class="step-desc">'+(s.k===nextKey?'Próxima etapa':s.optional?'Quando necessário':'')+'</span><span class="step-participants">'+participants(s)+'</span></div>';
    }).join('')+'</div>';
  }
  ...
  function projectLastAgent(pid){
    var ran=PROJ_RAN[pid]||[];return ran.length?ran[ran.length-1]:'';
  }
  function projectStateHTML(pid){
    var agent=projectLastAgent(pid),project=PROJECTS[pid]||{};
    if(!agent)return '<span class="stage-chip draft-chip">Projeto cadastrado</span>';
    var byStage={new:'Projeto cadastrado',diagnosed:'Potencial avaliado',elaborating:'Elaboração em andamento',elaborated:'Proposta elaborada',submitted:'Proposta submetida',complete:'Proposta concluída'};
    var label=byStage[project.stage]||({rico:'Oportunidades mapeadas',iris:'Potencial avaliado',ada:'Proposta elaborada',banca:'Proposta validada'}[agent]||'Etapa concluída');
    return '<button type="button" class="stage-chip project-result-chip" style="--sc:var(--c-'+agent+')" data-project-result="'+agent+'" data-proj="'+pid+'" aria-label="Abrir último resultado de '+nomeAgente(agent)+'"><svg aria-hidden="true"><use href="'+insigniaHref(agent)+'"/></svg><span>'+label+'<small>Último resultado · '+nomeAgente(agent)+'</small></span></button>';
  }
  ...
  function openProjectResult(agent,pid,source){
    ...
    if(agent==='ada'){openWs(pid);return}
    var index=agentIndexByKey(agent);if(index>-1)openResultLab(index,source,{actual:true,project:pid});
  }
  document.addEventListener('click',function(e){
    var trigger=e.target.closest('[data-project-result]');if(!trigger)return;
    e.preventDefault();openProjectResult(trigger.getAttribute('data-project-result'),trigger.getAttribute('data-proj'),trigger);
  });
  (function hydrateStaticProjectJourneys(){
    if(isFreshAccountMode)return;
    $$('#projListStaticWrap .proj-card').forEach(function(card,index){
      var pid='p'+(index+1);card.setAttribute('data-project-id',pid);
      var oldTimeline=card.querySelector('.stepper');if(oldTimeline)oldTimeline.outerHTML=projectJourneyHTML(pid);
      var caption=card.querySelector('.proj-title span');if(caption)caption.setAttribute('data-project-caption',pid);
      var oldState=card.querySelector('.proj-head>.stage-chip');if(oldState)oldState.outerHTML='<span data-project-state="'+pid+'">'+projectStateHTML(pid)+'</span>';
    });
  })();
  function projectNextActionsHTML(pid){
    ...
    if(hasAda){
      ...
      var ricoDesc=hasRico?'Use a escrita atualizada para buscar oportunidades mais aderentes.':'Use a escrita da Ada como contexto para buscar oportunidades aderentes.';
      var irisTitle=hasIris?'Atualizar diagnóstico com Íris':'Diagnosticar potencial com Íris';
      var irisDesc=hasIris?'Rode uma nova leitura se o projeto mudou antes da submissão.':'Revise a elegibilidade com base na escrita completa antes da submissão.';
      return '<div class="project-next-intro"><b>Próxima etapa: submissão da proposta</b><span>A proposta está elaborada. Prepare a versão final para submeter no canal oficial do edital.</span></div><button class="btn-primary project-agent-action project-agent-primary" data-ws-project="'+pid+'"><span><strong>Preparar submissão da proposta</strong><small>Abrir a versão final e concluir a conferência obrigatória</small></span></button><div class="project-human-review"><strong>Revisão humana obrigatória</strong><span>Antes de submeter, confira cada seção, os anexos e as regras do edital. A MonyU não submete nem garante aprovação.</span></div><div class="project-agent-refinements" aria-label="Recomendações complementares após a elaboração"><p>Com a escrita pronta, você também pode aprofundar:</p><button class="project-agent-refine" data-agent="rico" data-agent-detail-cta data-proj="'+pid+'"><span><strong>'+ricoTitle+'</strong><small>'+ricoDesc+'</small></span><em>'+costBadgeHTML('rico',1)+'</em></button><button class="project-agent-refine" data-agent="iris" data-agent-detail-cta data-proj="'+pid+'"><span><strong>'+irisTitle+'</strong><small>'+irisDesc+'</small></span><em>'+costBadgeHTML('iris',5)+'</em></button></div>';
    }
  ...
  function refreshProjectSpent(pid){
    ...
    $$('[data-project-spent="'+pid+'"]').forEach(function(el){
      el.innerHTML='<span class="ficha-coin" aria-hidden="true">M</span><b>'+spent+'</b> ficha'+(spent===1?'':'s')+' usada'+(spent===1?'':'s')+' por agentes';
    });
  }
  ...
  function openResultLab(i,source,context){
    var a=AGD[i],c=resultExampleConfig(a),hasProjects=Object.keys(PROJECTS).length>0;
    var actual=!!(context&&context.actual),projectName=context&&PROJECTS[context.project]&&PROJECTS[context.project].short;
    resultLabReturn=source||document.activeElement;
    resultLabInner.innerHTML='<div class="result-lab-shell" style="--accent:var(--c-'+a.k+')">'+
      '<main class="result-lab-main" id="resultLabScroll"><div class="result-lab-head"><div><span class="result-eyebrow">'+(actual?'Resultado mais recente - '+a.name:'Exemplo navegável - '+a.name)+'</span><h2 id="resultLabTitle">'+c.title+'</h2><p>'+(actual?'Resultado incorporado à memória de '+(projectName||'sua ideia/projeto')+'.':'Simulação de como o resultado chega para você.')+'</p></div><button class="result-lab-close" data-result-lab-close aria-label="Fechar resultado">✕</button></div>'+ 
  ...
  /* Nomes de agentes são sempre atalhos para o detalhe canônico. A conversão
     é aplicada ao texto renderizado e a cada inserção posterior, evitando que
     uma nova tela ou estado de projeto esqueça de criar esse acesso. Não mexe
     em controles que já possuem sua própria ação de agente. */
  function linkifyAgentNames(scope){
    var rootScope=scope||document.body;if(!rootScope||!AGD||!AGD.length)return;
    var names=AGD.filter(function(a){return isAllowedAgent(a.k)}).map(function(a){return a.name}).sort(function(a,b){return b.length-a.length});
    if(!names.length)return;
    var esc=function(v){return v.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')};
    var matcher=new RegExp('('+names.map(esc).join('|')+')','g');
    var nodes=[],walker=document.createTreeWalker(rootScope,NodeFilter.SHOW_TEXT,{acceptNode:function(node){
      if(!node.nodeValue||!matcher.test(node.nodeValue)){matcher.lastIndex=0;return NodeFilter.FILTER_REJECT}matcher.lastIndex=0;
      var parent=node.parentElement;
      return parent&&!parent.closest('button,a,input,textarea,select,option,script,style,svg,#agDrawer,#resultLab,#obOv,#plansModal,[data-agent-name-link]')?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT;
    }});
    while(walker.nextNode())nodes.push(walker.currentNode);
    nodes.forEach(function(node){
      var text=node.nodeValue,frag=document.createDocumentFragment(),last=0;
      text.replace(matcher,function(match,_name,offset){
        if(offset>last)frag.appendChild(document.createTextNode(text.slice(last,offset)));
        var agent=AGD.filter(function(a){return a.name===match})[0];
        if(agent){var link=document.createElement('button');link.type='button';link.className='agent-name-link';link.setAttribute('data-agent-name-link',agent.k);link.setAttribute('aria-label','Abrir detalhes de '+agent.name);link.textContent=match;frag.appendChild(link)}else frag.appendChild(document.createTextNode(match));
  ...
  document.addEventListener('click',function(e){
    var link=e.target.closest('[data-agent-name-link]');if(!link)return;
    e.preventDefault();e.stopPropagation();
    var index=agentIndexByKey(link.getAttribute('data-agent-name-link'));if(index>-1)openDrawer(index);
  });
  linkifyAgentNames(document.body);
  new MutationObserver(function(changes){changes.forEach(function(change){Array.from(change.addedNodes||[]).forEach(function(node){if(node.nodeType===1)linkifyAgentNames(node)})})}).observe(document.body,{childList:true,subtree:true});
  function canonicalRunSection(a,ctxProj){
    if(a.soon)return '';
    /* Sem projeto não existe execução a configurar. O próximo passo é criar a
       ideia/projeto, então a configuração e o custo não devem competir com esse
       CTA no primeiro contato. */
  ...
  function openDrawer(i,ctxProj){
    ...
    agInner.innerHTML=
      '<div class="drawer-head">'+
        '<span class="agent-identity" aria-hidden="true"><img class="agent-avatar" src="'+avatarSrc(a.k)+'" alt=""><svg class="av identity-insignia"><use href="'+insigniaHref(a.k)+'"/></svg></span>'+
   ...
   estas duas grades continuavam com 2 colunas mesmo a 320px */
      ...
      '<div class="drawer-foot">'+
        (a.soon
          ...
          : (hasProjects
              ...
              : '<button class="btn-primary new-project-primary free-project-cta-empty" id="runProjNew">Cadastrar nova ideia/projeto <span class="free-action-badge">GRÁTIS</span></button>')+
    ...
    agInner.querySelectorAll('.t-view').forEach(function(b){
      b.addEventListener('click',function(){
        if(a.k==='rico'){closeDrawer();showView('radar')}
        else if(a.k==='ada'){closeDrawer();openWs()}
        else{toast('Abrindo o resultado - visualização completa em breve no piloto (demo)')}
      });
    });
  ...
  function sourcesSec(a){
    ...
    return '<div class="dsec"><h4>Fontes de pesquisa</h4>'+
      (isMvpMode?'<div class="src-row"><span>📚 Base oficial MonyU - editais, regras e templates validados</span><b>sempre ativa</b></div>':'<div class="src-row"><span>📚 Núcleo MonyU - editais, jurisprudência e templates validados</span><b>sempre ativo</b></div>')+
      (isMvpMode?'':'<div class="src-row"><span>🗂 Seu Memória - global + projeto do contexto</span><b>sempre ativo</b></div>')+
  ...
  if(isFreshAccountMode)DOCS.length=0; /* conta nova: Memória começa vazio, nada foi indexado ainda */
  ...
  function renderKb(){
    ...
    Object.keys(PROJECTS).forEach(function(pid){
      folders+='<button class="kb-folder'+(kbCur===pid?' on':'')+'" data-kb="'+pid+'">'+
        projIconHTML(PROJECTS[pid].name,PROJECTS[pid].tema)+
  ...
  function agLink(k,label){
    return '<button class="ag-link" data-agkey="'+k+'" style="--alc:var(--c-'+k+')"><svg class="av" aria-hidden="true"><use href="'+insigniaHref(k)+'"/></svg>'+label+'</button>';
  }
  var TIPS=[
  ...
  if(isMvpMode)TIPS=[
    'O <b>Rico</b> encontra as oportunidades; a <b>Íris</b> mostra se vale a pena competir; e a <b>Ada</b> elabora o projeto com revisão humana antes da submissão.',
    'Comece pelo seu projeto. O Rico encontra oportunidades aderentes e você decide quando avançar para o diagnóstico da Íris.',
    'A Ada não submete nada. Ela prepara o projeto para sua revisão humana e para o envio no portal oficial.'
  ];
  ...
  function relSec(a){
    var rels=(REL[a.k]||[]).filter(function(r){return isAllowedAgent(r.k)});
    ...
    return '<div class="dsec"><h4>Funciona melhor em conjunto</h4>'+
      rels.map(function(r){
        ...
        return '<button class="rel-row" data-relk="'+r.k+'">'+
          '<svg class="av" aria-hidden="true"><use href="'+insigniaHref(r.k)+'"/></svg>'+
...
'<div class="key-stats ks2" style="margin-bottom:1.15rem">'+
  ...
  function obAgent(k,name,what,value,cost,state){
    var st=state==='on'?'<span class="ob-state on">✓ Disponível no acesso</span>'
      :state==='beta'?'<span class="ob-state on">✓ Disponível · revisão humana</span>'
    ...
    return '<div class="ob-agent" style="--ac:var(--c-'+k+')">'+
      '<span class="ob-agent-identity"><img class="agent-avatar" src="'+avatarSrc(k)+'" alt=""><svg class="av" aria-hidden="true"><use href="'+insigniaHref(k)+'"/></svg></span>'+
  ...
  function obMvpAgent(k,role,cost){
    var name=(AG[k]&&AG[k].name)||k;
    return '<article class="ob-mvp-agent" style="--ac:var(--c-'+k+')">'+
      '<span class="ob-agent-identity"><img class="agent-avatar" src="'+avatarSrc(k)+'" alt="Retrato de '+name+'"><svg class="av" aria-hidden="true"><use href="'+insigniaHref(k)+'"/></svg></span>'+
      '<span class="ob-mvp-agent-copy"><b>'+name+'</b><small>'+role+'</small></span>'+
      '<span class="ob-mvp-cost"><span class="ficha-coin">M</span>'+cost+'</span>'+
    '</article>';
  }
  var OB_STEPS=[
    ...
    {h:'A jornada: do edital certo ao projeto pronto 🛣',
     b:'<p>Cinco passos centrais organizam a captação. Bartô entra como gate condicional quando a decisão exige viabilidade integrada:</p>'+
       '<div class="ob-avrow"><svg class="av"><use href="#ins-rico"/></svg>→<svg class="av"><use href="#ins-iris"/></svg>→<svg class="av"><use href="#ins-carlito"/></svg>→<svg class="av"><use href="#ins-ada"/></svg>→<svg class="av"><use href="#ins-banca"/></svg></div><div class="ob-note" style="margin-bottom:.65rem"><b>Gate Bartô:</b> depois de Aurora, Íris ou Kai e antes de Carlito/Ada quando houver capital, contrapartida, novidade ou risco material.</div>'+
    ...
    {h:'Eles trabalham em paralelo - você vive sua vida ⚡',
     b:'<div class="ob-mock-tray"><div class="tray-head"><span class="spin"></span>Atividade dos agentes <span style="color:var(--text-3);font-weight:600;font-size:.72rem">· 3 em execução</span></div>'+
       '<div class="tray-item"><svg class="av"><use href="#ins-rico"/></svg><div class="t-body"><div class="t-label">Rico - scan de oportunidades · Todos os projetos</div><div class="tray-prog"><i></i></div></div></div>'+
       '<div class="tray-item"><svg class="av"><use href="#ins-iris"/></svg><div class="t-body"><div class="t-label">Íris - diagnóstico · Plataforma IoT</div><div class="tray-prog"><i></i></div></div></div>'+
       '<div class="tray-item"><svg class="av"><use href="#ins-ada"/></svg><div class="t-body"><div class="t-label">Ada - elaboração · Microrredes solares</div><div class="tray-prog"><i></i></div></div></div></div>'+
  ...
  if(isMvpMode)OB_STEPS=[
    {h:'Bem-vindo(a) ao Acesso Beta Antecipado',
     next:'Conhecer os 3 agentes →',
     b:'<div class="ob-hero"><a href="https://www.monyu.com.br" target="_blank" rel="noopener noreferrer" aria-label="Abrir o site oficial da MonyU em nova janela"><svg width="84" height="84" aria-hidden="true"><use href="#monyu-symbol"/></svg></a></div><p>Nesta fase, você conta com Rico, Íris e Ada para transformar uma ideia em um projeto mais preparado para captação.</p><div class="ob-note">Comece quando quiser: cadastrar uma ideia/projeto é gratuito e leva poucos minutos.</div><div style="display:flex;gap:.6rem;margin-top:1rem;flex-wrap:wrap"><button class="btn-primary" id="obZeroBtn" style="flex:1">Cadastrar minha ideia/projeto <span class="free-action-badge">GRÁTIS</span></button></div>'},
    {h:'Sua equipe para captar melhor',
     next:'Ver como usar fichas →',
     b:'<p>Três assistentes de IA, cada um com uma responsabilidade clara. Você vê o custo antes de confirmar qualquer execução.</p><div class="ob-mvp-agents">'+obMvpAgent('rico','Mapeia oportunidades aderentes','1 a 3 por rodada')+obMvpAgent('iris','Diagnostica o potencial de captação','5 por projeto')+obMvpAgent('ada','Elabora o projeto para o edital','20 por projeto')+'</div><div class="ob-note">A Ada prepara o material. A revisão humana acontece antes de qualquer submissão.</div>'},
    {h:'Fichas ativam o trabalho dos agentes',
     next:'Ver como você fica no comando →',
     b:'<div class="ob-hero"><span class="ficha-coin" style="width:64px;height:64px;font-size:1.6rem">M</span></div><p>Fichas são usadas apenas quando você ativa uma execução. O custo e o saldo depois da ação aparecem antes da confirmação.</p><div class="ob-ficha-rules"><span><b>1</b> Você escolhe a ação</span><span><b>2</b> Confere custo e saldo</span><span><b>3</b> Confirma a execução</span></div><div class="ob-note">No Acesso Beta Antecipado, todas as execuções seguem essa mesma lógica simples.</div>'},
  ...
  function renderObTeamRow(){
    ...
    row.innerHTML=pool.slice(0,5).map(function(a){
      return '<span class="ob-team-item"><img class="agent-avatar" src="'+avatarSrc(a.k)+'" alt="Retrato de '+a.name+'" loading="lazy"><span>'+a.name+'</span></span>';
    }).join('');
  }
  ...
  if(store.get('onboarded','')!=='1'){
    if(isMvpMode){
      MVP_OPEN_ONBOARD_AFTER_MILESTONE=mvpCelebrate('first-access','Bem-vindo(a) ao Acesso Beta Antecipado!','Este é o começo da sua jornada para transformar ideias em projetos mais preparados para captação.','👋');
      if(!MVP_OPEN_ONBOARD_AFTER_MILESTONE)setTimeout(function(){openOb(0)},600);
    }else setTimeout(function(){openOb(0)},600);
  }
  ...
  function openNextSteps(pid){
    ...
    $('#nsList').innerHTML=[
      ...
      return '<div class="match-row" style="cursor:default">'+
        '<svg class="av" style="width:36px;height:36px" aria-hidden="true"><use href="#ins-'+x[0]+'"/></svg>'+
  ...
  function openMvpAdvisorWhatsApp(origin){
    var message='Olá, equipe MonyU! Estou participando do Acesso Beta Antecipado da Plataforma Agêntica MonyU e tenho interesse em saber mais sobre o serviço de assessoria especializada para captação de recursos. Podemos conversar?';
    window.open('https://wa.me/558599991111?text='+encodeURIComponent(message),'_blank','noopener,noreferrer');
    track('mvp_assessoria_whatsapp',{origem:origin||'sidebar'});
  }
  $('#ctaSpec').addEventListener('click',function(){if(isMvpMode){openMvpAdvisorWhatsApp('sidebar');return}openSpecModal(null)});
  var wsSpecBtn=$('#wsSpecBtn');
  if(wsSpecBtn)wsSpecBtn.addEventListener('click',function(){if(isMvpMode){openMvpAdvisorWhatsApp('workspace_ada');return}openSpecModal('p1')});
  ...
  if(isFreshAccountMode)commands=commands.filter(function(c){return !c.zeroSkip});
  if(isMvpMode)commands=commands.filter(function(c){return !/Memória|GigaMonyU|Treinamentos|especialista MonyU|Aurora|Kai|Mike/.test(c.t)});
  if(isMvpMode)commands.push({t:'Conversar sobre assessoria especializada',run:function(){openMvpAdvisorWhatsApp('paleta_comandos')}});
  ...
  function renderAtividade(){
    ...
    lista.innerHTML='<div class="card">'+eventos.map(function(e){
      return '<div class="v29-ev">'+
        '<svg class="av" aria-hidden="true"><use href="'+insigniaHref(e.ag)+'"/></svg>'+
  ...
  var HOME_EVENTOS=[
    ...
    {ag:'rico',txt:'<b>Rico</b> encontrou 3 oportunidades novas - a melhor com <b>92% de aderência</b>',
     ...
     org:'auto',orgTxt:'AUTO',custo:'1 ficha',quando:'hoje, 08:12'},
    {ag:'ada',txt:'<b>Ada</b> deixou metodologia e cronograma em blocos prontos para copiar',
     why:'Você pediu a elaboração no dia 30. O texto fica aqui para você revisar e colar no formulário - a MonyU não submete nada.',
     org:'voce',orgTxt:'MANUAL',custo:'20 fichas',quando:'ontem, 11:05'}
  ];
  var MVP_HOME_EVENTOS=[];
  function renderHomeAtividade(){
    var box=$('#homeAtividade'); if(!box)return;
    var eventos=isMvpMode?MVP_HOME_EVENTOS:HOME_EVENTOS;
    if(!eventos.length){box.innerHTML='<div class="home-ev"><span><span class="home-ev-txt"><b>Nenhuma atividade ainda.</b><span class="home-ev-why">Cadastre uma ideia/projeto. As análises, recomendações e execuções aparecerão aqui.</span></span></span></div>';return}
    box.innerHTML=eventos.map(function(e){
      return '<div class="home-ev">'+
        ...
        '</span></div>';
    }).join('')+(isMvpMode?'':'<div class="home-auto-nota"><span>No automático desde 12 jul · '+
  ...
  function priorizarAvisos(){
    ...
    if(isMvpMode&&Object.keys(PROJECTS).length===0){slots.forEach(function(el){el.style.display='none'});return}
  ...
  /* Feedback global: anônimo por padrão. Perfil agregado sem identificadores
     diretos; nome, e-mail e organização só entram após escolha explícita. */
  (function(){
    var trigger=$('#feedbackBtn'),modal=$('#feedbackModal'),form=$('#feedbackForm'),textInput=$('#feedbackText'),count=$('#feedbackCount'),status=$('#feedbackStatus'),identify=$('#feedbackIdentify'),preview=$('#feedbackIdentityPreview'),privacyTitle=$('#feedbackPrivacyTitle'),privacyDesc=$('#feedbackPrivacyDesc'),submit=$('#feedbackSubmit'),terms=$('#feedbackTerms'),termsHint=$('#feedbackTermsHint'),termsOpen=$('#feedbackTermsOpen'),termsModal=$('#feedbackTermsModal'),termsAccept=$('#feedbackTermsAccept'),feature=$('#feedbackFeature'),activeFeature=$('#feedbackActiveFeature'),screenLabel=$('#feedbackScreenLabel'),TERMS_VERSION='2026-09-14';
    if(!trigger||!modal||!form)return;
    function fieldValue(id,fallback){var el=$(id);return el&&el.value?el.value.trim():fallback}
    function browserFamily(){var ua=navigator.userAgent.toLowerCase();if(ua.indexOf('edg/')>-1)return'Edge';if(ua.indexOf('opr/')>-1)return'Opera';if(ua.indexOf('firefox/')>-1)return'Firefox';if(ua.indexOf('safari/')>-1&&ua.indexOf('chrome/')===-1)return'Safari';if(ua.indexOf('chrome/')>-1)return'Chrome';return'Outro'}
    function range(value){if(value===0)return'0';if(value===1)return'1';if(value<=5)return'2-5';return'6+'}
    function profile(){return{access:accessMode||'full',language:document.documentElement.lang||'pt-BR',device:window.innerWidth<700?'celular':(window.innerWidth<1024?'tablet':'desktop'),browser:browserFamily(),projects:range(Object.keys(PROJECTS||{}).length),executions:range($$('#executionLog .exec-log-row').length)}}
    function identity(){return{name:fieldValue('#pfNome',''),email:fieldValue('#pfEmail',''),organization:fieldValue('#empFantasia','')}}
    function activeContext(){var active=$('.view.active'),view=active?active.id.replace('view-',''):'unknown',labels={home:'Início',projetos:'Projetos',radar:'Oportunidades',aprovacoes:'Central de Aprovações',agentes:'Agentes',memoria:'Memória',perfil:'Perfil',planos:'Planos',ajuda:'Ajuda e FAQ'};return{view:view,label:labels[view]||'Tela atual'}}
    function syncContext(){var context=activeContext();if(screenLabel)screenLabel.textContent='Você está em '+context.label;if(activeFeature){activeFeature.value='tela:'+context.view;activeFeature.textContent='Esta tela - '+context.label}return context}
    function termsRecord(){try{var saved=JSON.parse(store.get('feedback-terms',''));return saved&&saved.version===TERMS_VERSION?saved:null}catch(err){return null}}
    function saveTerms(){var record={version:TERMS_VERSION,acceptedAt:new Date().toISOString()};store.set('feedback-terms',JSON.stringify(record));return record}
    function syncTerms(){var accepted=termsRecord();terms.checked=!!accepted;submit.disabled=!accepted;termsHint.textContent=accepted?'Termos aceitos em '+new Date(accepted.acceptedAt).toLocaleDateString('pt-BR')+'. O aceite vale para esta versão.':'Leia e aceite os termos para enviar. Quando a versão mudar, pediremos novo aceite.';terms.closest('.feedback-terms').classList.toggle('is-required',!accepted)}
    function syncIdentity(){var on=!!identify.checked,id=identity();preview.hidden=!on;privacyTitle.textContent=on?'Feedback identificado':'Seu feedback começa anônimo';privacyDesc.textContent=on?'Nome, e-mail e organização do seu perfil serão anexados.':'Sem nome, e-mail, empresa ou conteúdo dos seus projetos.';submit.textContent=on?'Enviar identificado':'Enviar anonimamente';$('#feedbackIdentityName').textContent=id.name||'Perfil sem nome';$('#feedbackIdentityMeta').textContent=[id.email,id.organization].filter(Boolean).join(' · ');$('#feedbackIdentityAvatar').textContent=(id.name||'U').split(/\s+/).slice(0,2).map(function(part){return part.charAt(0)}).join('').toUpperCase()}
    trigger.addEventListener('click',function(){status.textContent='';identify.checked=false;syncIdentity();syncTerms();syncContext();open(modal);setTimeout(function(){var first=form.querySelector('input[name="feedbackType"]');if(first)first.focus()},0)});
    identify.addEventListener('change',syncIdentity);syncIdentity();syncTerms();
    terms.addEventListener('change',function(){if(terms.checked)saveTerms();else store.set('feedback-terms','');syncTerms()});
    termsOpen.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();open(termsModal)});
    termsAccept.addEventListener('click',function(){terms.checked=true;saveTerms();syncTerms();close(termsModal);terms.focus()});
    textInput.addEventListener('input',function(){count.textContent=textInput.value.length});
    form.addEventListener('submit',function(e){
      e.preventDefault();
      var type=form.querySelector('input[name="feedbackType"]:checked'),impact=form.querySelector('input[name="feedbackImpact"]:checked');
      if(!type||!textInput.value.trim())return;
      if(!terms.checked){termsHint.textContent='Você precisa aceitar os termos antes de enviar.';terms.closest('.feedback-terms').classList.add('is-required');terms.focus();return}
      var context=syncContext(),selectedFeature=feature&&feature.options[feature.selectedIndex],anonymous=!identify.checked;
      var payload={id:'fb-'+Date.now(),anonymous:anonymous,type:type.value,impact:impact?impact.value:'medio',feature:{id:selectedFeature?selectedFeature.value:'tela-atual',label:selectedFeature?selectedFeature.textContent:'Esta tela'},message:textInput.value.trim(),createdAt:new Date().toISOString(),terms:termsRecord()||saveTerms(),profile:profile(),context:{view:context.view,viewLabel:context.label,version:document.body.getAttribute('data-app-version')||'v30'}};
      if(!anonymous)payload.identity=identity();
      var saved=[];try{saved=JSON.parse(store.get('feedback','[]'))||[]}catch(err){saved=[]}
      saved.unshift(payload);store.set('feedback',JSON.stringify(saved.slice(0,50)));
      var celebrateFeedback=isMvpMode&&store.get('mvp-milestone-primeiro-feedback','')!=='1';if(!isMvpMode)rwUnlock('primeiro-feedback');
      status.textContent=anonymous?'Feedback anônimo registrado. Obrigado por ajudar a melhorar a MonyU.':'Feedback identificado registrado. Obrigado por ajudar a melhorar a MonyU.';
      form.reset();count.textContent='0';syncIdentity();setTimeout(function(){close(modal);if(celebrateFeedback)setTimeout(function(){mvpCelebrate('primeiro-feedback','Sua voz constrói','Seu primeiro feedback foi enviado. Obrigado por ajudar a melhorar a MonyU.','💬')},180)},900);
    });
  })();
  ...
  (function(){
    ...
    function hideNudge(remember){if(nudge)nudge.hidden=true;if(remember)store.set('install-nudge-dismissed',String(Date.now()))}
